...
// var defer = new DeferredTask(start, 100);
...
function portEvents(worker) {
	...
	*  ToDO: encoding types */
	...
	on('get-paths', function(data){
		getPaths(worker, callback);
	});
	return;
}

function getPaths (worker, callback) {
	var domain = worker.url.match(/\/\/([^\/?]*)/)[1];
	paths.getDomainPaths(domain, function(paths){
		console.log(paths);
		callback(paths);
		// worker.port.emit('got-paths', paths);
	});
}
